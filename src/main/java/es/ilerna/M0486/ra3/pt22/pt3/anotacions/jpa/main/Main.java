...
import es.ilerna.M0486.ra3.pt22.pt3.anotacions.jpa.domain.*;
	...
	private static void fase2() {
	    ...
	    if (v != null) {
	        v.setOwner(null); // Desvincula la persona [cite: 35, 37]
	        session.update(v);
	    }
	...
	private static void fase3() {
	    ...
	    try {
	        Vehicle v = session.get(Vehicle.class, 1);
	        if (v != null) {
	            v.setBrand("Seat");      // Correcte: Seat [cite: 44]
	            v.setPrice(19999.0);     // Correcte: 19.999 [cite: 44]
	            v.setYear(2022);         // Correcte: 2.022 [cite: 44]
	            session.update(v);
	        }
	        tx.commit();
